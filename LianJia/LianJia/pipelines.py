...
class LianjiaPipeline(object):
    ...
    def process_item(self, item, spider):   # 转化写入文件
        ...
        # 将字典转化为字符串
        item1 = item
        data = json.dumps(dict(item1),ensure_ascii=False)+'\n'
...
class ImagePipeline(ImagesPipeline):
    ...
    def item_completed(self, results, item, info):
        ...
        print image, '============='
        for i in range(len(item['image_src'])):
            # 拼接　路径名字
            old_name = self.IMAGES_STORE + os.sep +image[0]
            print old_name,"$$$$$$$$$$$$$$$$$$$$$$$$"
            new_name = self.IMAGES_STORE + os.sep + image[0].split(os.sep)[0] +os.sep + item['name'] + str(i) + '.jpg'
            print new_name,'&&&&&&&&&&&&'
